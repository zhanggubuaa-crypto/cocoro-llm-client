...
**エラー内容**:
```
curl: (7) Failed to connect to 192.168.50.112 port 4000: Connection refused
```
...
**エラー内容**:
```
curl: (7) Failed to connect to 192.168.50.112 port 4000: No route to host
```
...
```bash
# WSL でWindowsのIPアドレスを確認
cat /etc/resolv.conf | grep nameserver | awk '{print $2}'

# または、/etc/hosts に追加
echo "192.168.50.112 host.docker.internal" | sudo tee -a /etc/hosts
```
...
**エラー内容**:
```
curl: (6) Could not resolve host: 192.168.50.112
```
...
```bash
cat /etc/hosts | grep 192.168.50.112
```
...
```bash
curl http://192.168.50.112:8000/v1/models | jq '.data[].id'
```
...
```bash
# curl で直接テスト
curl -v http://192.168.50.112:4000/health/liveliness
curl -v http://192.168.50.112:4000/v1/models
```
